...
module.exports = [
	...
	class DeleteChannelCommand extends Command {
		...
		async run(bot, message, args, flags) {
			...
			if (channel.createdTimestamp + 1.5552e+10 < Date.now() && !flags.some(f => f.name == "yes")) {
				const promptRes = await prompter.prompt(message,
	...
	class PurgeCommand extends Command {
		...
		async run(bot, message, args, flags) {
			...
			if (args[0] > 100) {
				const promptRes = await prompter.prompt(message, `You are about to delete up to ${args[0]} messages from this channel.`);
				if (promptRes.error) return {cmdWarn: promptRes.error};
