...
const prompter = require("../modules/codePrompter.js");
	...
	class DeleteChannelCommand extends Command {
		...
		async run(bot, message, args, flags) {
			...
			if (channel.createdTimestamp + 1.5552e+10 < Number(new Date())) {
				let cmdErr = await prompter.prompt(message, `You are about to delete the channel **${channel.name}**, which is more than 180 days old.`)
				if (cmdErr) return message.channel.send(cmdErr);
			}
	...
	class DeleteRoleCommand extends Command {
		...
		async run(bot, message, args, flags) {
			...
			if (role.members.size > 10 && role.members.size > message.guild.memberCount / 10) {
				let cmdErr = await prompter.prompt(message, `You are about to delete the role **${role.name}**, which more than 10% of the members in this server have.`)
				if (cmdErr) return message.channel.send(cmdErr);
			}
