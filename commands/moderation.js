...
module.exports = [
	...
	class BanCommand extends Command {
		...
		async run(bot, message, args, flags) {
			...
			let cmdErr = await prompter.prompt(message, `You are about to ban the user **${member.user.tag}** from this server.`);
	...
	class DeleteChannelCommand extends Command {
		...
		async run(bot, message, args, flags) {
			...
			if (channel.createdTimestamp + 1.5552e+10 < Number(new Date()) && !flags.some(f => f.name == "yes")) {
				let cmdErr = await prompter.prompt(message, `You are about to delete the channel **${channel.name}** (ID ${channel.id}), which is more than 180 days old.`)
				if (cmdErr) return message.channel.send(cmdErr);
			}
	...
	class DeleteRoleCommand extends Command {
		...
		async run(bot, message, args, flags) {
			...
			if (role.members.size > 10 && role.members.size > message.guild.memberCount / 10) {
				let cmdErr = await prompter.prompt(message, `You are about to delete the role **${role.name}** (ID ${role.id}), which more than 10% of the members in this server have.`)
				if (cmdErr) return message.channel.send(cmdErr);
			}
	...
	class KickCommand extends Command {
		...
		async run(bot, message, args, flags) {
			...
			let cmdErr = await prompter.prompt(message, `You are about to kick the user **${member.user.tag}** from this server.`)
			if (cmdErr) return message.channel.send(cmdErr);
