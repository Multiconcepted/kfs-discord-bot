const {Permissions} = require("discord.js"),
	...
	prompter = require("../modules/codePrompter.js");
...
module.exports = [
	...
	class BanCommand extends Command {
		...
		async run(bot, message, args, flags) {
			...
			if (!flags.some(f => f.name == "yes")) {
				const promptRes = await prompter.prompt(message, `You are about to ban the user **${member.user.tag}** from this server.`);
	...
	class DeleteChannelCommand extends Command {
		...
		async run(bot, message, args, flags) {
			...
			if (channel.createdTimestamp + 1.5552e+10 < Number(new Date()) && !flags.some(f => f.name == "yes")) {
				const promptRes = await prompter.prompt(message, `You are about to delete the channel **${channel.name}** (ID ${channel.id}), which is more than 180 days old.`);
				if (promptRes) return {cmdWarn: promptRes};
			}
	...
	class DeleteRoleCommand extends Command {
		...
		async run(bot, message, args, flags) {
			...
			if (role.members.size > 10 && role.members.size > message.guild.memberCount / 10 && !flags.some(f => f.name == "yes")) {
				const promptRes = await prompter.prompt(message, `You are about to delete the role **${role.name}** (ID ${role.id}), which more than 10% of the members in this server have.`);
				if (promptRes) return {cmdWarn: promptRes};
			}
	...
	class KickCommand extends Command {
		...
		async run(bot, message, args, flags) {
			...
			const promptRes = await prompter.prompt(message, `You are about to kick the user **${member.user.tag}** from this server.`);
			if (promptRes) return {cmdWarn: promptRes};
	...
	class PurgeCommand extends Command {
		...
		async run(bot, message, args, flags) {
			...
			if (args[1] || flags.length > 0) {
				...
				const promptRes = await prompter.prompt(message, `You are about to delete ${toDelete} messages from this channel.`);
				if (promptRes) return {cmdWarn: promptRes};
	...
	class SoftbanCommand extends Command {
		...
		async run(bot, message, args, flags) {
			...
			if (!flags.some(f => f.name == "yes")) {
				const promptRes = await prompter.prompt(message, `You are about to softban the user **${member.user.tag}** in this server.`);
				if (promptRes) return {cmdWarn: promptRes};
			}
